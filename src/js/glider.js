...
(() => {
  ...
  mount.innerHTML = `<section class="glider-win is-splash" id="glider-win" hidden aria-label="Glider 2026" tabindex="-1">
    ...
    <div class="glider-stage">
      ...
      <canvas width="800" height="500" aria-label="Paper plane game: use left and right to glide, ride air vents upward and avoid furniture and the floor."></canvas>
      <div class="glider-hud" aria-label="Game status"><span id="glider-room" class="glider-hud-box">Tokyo Blue Note</span><span class="glider-cards"><b id="glider-room-number" title="Room">1</b><b id="glider-floor-number" title="Floor">1</b></span><span id="glider-score" class="glider-hud-box" title="Score">000000</span><span class="glider-hud-box glider-supplies"><span class="glider-fuel" title="Battery energy"><span id="glider-fuel">0</span><i class="glider-battery" aria-hidden="true"></i></span><span id="glider-lives" aria-label="3 lives"></span></span></div>
    ...
    <div class="glider-footer"><div class="glider-controls"><button data-key="left" aria-label="Fly left">←</button><button data-key="right" aria-label="Fly right">→</button></div><div class="glider-actions"><button class="glider-pause" aria-label="Pause game">Ⅱ</button><button class="glider-reset" aria-label="Restart game">↻</button></div></div>
  </section>`;
  ...
  const rooms = [
    { name:'01 / TOKYO · BLUE NOTE', color:'#6dcce3', floor:230, door:[154,210], vents:[{x:43,w:40,top:113},{x:179,w:42,top:63},{x:307,w:38,top:115}],
      blocks:[{x:87,y:191,w:73,h:33},{x:108,y:170,w:49,h:21},{x:174,y:186,w:54,h:35},{x:280,y:202,w:25,h:21},{x:320,y:202,w:25,h:21},{x:353,y:182,w:12,h:39}] },
    { name:'02 / PARIS · MINUIT', color:'#eabf7c', floor:209, door:[109,192], vents:[{x:47,w:40,top:45},{x:179,w:42,top:80},{x:309,w:40,top:58}],
      blocks:[{x:77,y:166,w:38,h:36},{x:148,y:139,w:26,h:63},{x:161,y:116,w:5,h:23},{x:203,y:155,w:79,h:47},{x:314,y:166,w:29,h:36}] }
  ];
  let room=0, lives=3, mode='intro', plane;
  let frame=0, last=0, elapsed=0, invincible=0;
  const keys={left:false,right:false};
  function resetPlane() { plane={x:22,y:124,vx:0,vy:0,facing:1}; invincible=1.5; }
  function reset() { room=0;lives=3;elapsed=0;resetPlane(); }
  reset();
  ...
  function scenery() {
    ...
    rooms[room].vents.forEach(v=>{
      const bottom=rooms[room].floor-4,height=bottom-v.top;
      for(let i=0;i<6;i++){
  ...
  function draw() {
    ...
    score.textContent=String(0).padStart(6,'0');roomLabel.textContent=mode==='won'?'Flight complete!':mode==='over'?'Out of planes':room===0?'Tokyo Blue Note':'Le Minuit, Paris';
    ...
    win.querySelector('#glider-floor-number').textContent=1;
    win.querySelector('#glider-fuel').textContent=0;
  ...
  function update(dt) {
    ...
    // Glider 4.05 G-PlayActive.p: holdKey uses ±5 px/frame or 0;
    // Coordinate supplies lift 2, MoveGlider adds gravity 1; vents set -7.
    // Original cadence is 30 Hz. Scale 512×342 coordinates to this room canvas.
    if(direction)plane.facing=direction;
    plane.vx=direction*5*30*(400/512);
    const onVent=rooms[room].vents.some(v=>plane.x>v.x && plane.x<v.x+v.w && plane.y>v.top);
    plane.vy=(onVent?-6:3)*30*(250/342);
    plane.x+=plane.vx*dt;plane.y+=plane.vy*dt;
    if(plane.y<19){plane.y=19;plane.vy=7;}
    if(plane.x<12){if(room===1 && plane.y>rooms[room].door[0] && plane.y<rooms[room].door[1]){room=0;plane.x=379;plane.y=168;invincible=.8;}else{plane.x=12;plane.vx=0;}}
    if(plane.x>387){
      if(plane.y>rooms[room].door[0] && plane.y<rooms[room].door[1]){
        if(room===0){room=1;plane.x=22;invincible=.8;}
        else {clearKeys();mode='won';overlay.hidden=true;win.querySelector('.glider-reset').focus({preventScroll:true});}
      }else{plane.x=387;plane.vx=0;}
    }
  ...
  function setPaused(){if(mode==='playing'){mode='paused';clearKeys();show('Intermission','Arrow keys / A D: move left and right. Ride the air vents. Avoid furniture and the floor.','RESUME →');}else if(mode==='paused'){mode='playing';overlay.hidden=true;}pause.textContent=mode==='paused'?'▶':'Ⅱ';}
